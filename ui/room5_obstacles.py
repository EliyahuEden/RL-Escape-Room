...
def render() -> None:
    ...
    def make_env(s=None):
        return ObstacleEnv(n_obstacles=n_obs, sensor_range=sensor_range,
                           max_sensed=max_sensed, randomize=True, seed=s)
    ...
    if train:
        ...
        st.session_state["room5"] = result
        st.session_state.pop("room5_test", None)

    result = st.session_state.get("room5")
    with right:
        st.subheader("Result")
        if not result:
    ...
    if result:
        ...
        with c1:
            ...
            if st.button("Generate & test"):
                test_env = make_env(int(test_seed))
        ...
        if test_frames:
            replay_player({"Random test room": test_frames},
                          lambda f: _draw(env, f), key="room5_test",
                          caption=f"Greedy policy in an unseen room — "
                                  f"ended: {test_frames[-1].get('event', '—')}")

        st.subheader("🎬 Replay training-stage episodes")
        replay_player(result.snapshots, lambda f: _draw(env, f), key="room5",
